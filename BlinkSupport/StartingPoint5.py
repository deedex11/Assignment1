...
(xTrain, xTest) = Assignment5Support.Featurize(xTrainRaw, xTestRaw, includeGradients=True, includeRawPixels=False, includeIntensities=True)
yTrain = yTrainRaw
yTest = yTestRaw
...
model = NeuralNetModel.NeuralNetModel()
for numHiddenLayer in [1, 2]:
	for numNodePerHiddenLayer in [2, 5, 10, 15, 20]: 
		...
		model.fit(np.array(xTrain), np.array(yTrain), .05, numHiddenLayer, numNodePerHiddenLayer, 1)
